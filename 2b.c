...
void process_packet(u_char *user, const struct pcap_pkthdr *header, const unsigned char *packet) {
    ...
    const char *payload_data = (const char *)(packet + tcp_data_offset);


    if (strstr(payload_data, "secret") != NULL) {
        ...
        printf("HTTP Payload Data:\n");
        printf("%.*s\n\n", (int)(header->caplen - tcp_data_offset), payload_data);
    }
}
